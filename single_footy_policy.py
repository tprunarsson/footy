...
- Loads a single player's Q-slab saved by qfooty_python_port.py
  shape: [n, m, n, m, na] where na is **4 or 5**.
    • 4 actions: N,E,S,W (move only)
    • 5 actions: N,E,S,W,Stay (Stay = do nothing)
- Chooses actions via epsilon-greedy on Q[ip, jp, ib, jb, :].
- Validates moves against the pitch; greedy selection **ignores illegal moves**.
- Optional `stay_debit` lets you **penalize Stay** during selection & display (for debugging/tiebreaks).
- Optional debug printing of per-action Q-values, legal moves, the debit-adjusted "effective" Q, and the chosen action.
...
class SingleFooty:
    def __init__(self, n: int = 13, m: int = 9, epsilon: float = 0.1, rng_seed: int | None = None,
        ...
        self.Q: np.ndarray | None = None  # shape [n,m,n,m,na]
    ...
    def load(self, path: str) -> bool:
        arr = np.load(path)
        if arr.ndim != 5 or arr.shape[0] != self.n or arr.shape[1] != self.m or arr.shape[2] != self.n or arr.shape[3] != self.m:
    ...
    def action(self, ip: int, jp: int, io: int, jo: int, ib: int, jb: int) -> int:
        ...
        Parameters (match C++ signature for convenience):
            ...
            io, jo : opponent position (not used here)
